...
screenMode = 0  # 0 = screen 1, 1 = screen 2
lastFlip = time.time()
...
def parseGPS():
    try:
        ...
        if readFix != 0:
            ...
            # Parse Altitude
            alt_str = NMEAmain['GPGGA'].split(',')[9]
            if alt_str:
                GPSdata['altitude'] = float(alt_str)
            else:
                GPSdata['altitude'] = 0
            ...
            # Parse Date and Time
            utc_time = NMEAmain['GPRMC'].split(',')[1]
            utc_date = NMEAmain['GPRMC'].split(',')[9]
            
            if utc_time and utc_date:
                hour = int(utc_time[0:2]) + 5
                if hour >= 24:
                    hour -= 24
...
def dispOLED():
    global screenMode, lastFlip
    
    # Check if 10 seconds passed, flip screen
    if time.time() - lastFlip >= 10:
        screenMode = 1 - screenMode  # Toggle between 0 and 1
        lastFlip = time.time()
    ...
    if GPSdata['fix'] == False:
        ...
        if screenMode == 0:
            # Screen 1: Date, Time, Lat, Lon
            dsp.text("Date: " + str(GPSdata['date']), 0, 0)
            dsp.text("Time: " + str(GPSdata['time']), 0, 12)
            dsp.text("LAT: " + str(round(GPSdata['latDD'], 4)), 0, 24)
            dsp.text("LON: " + str(round(GPSdata['lonDD'], 4)), 0, 36)
        else:
            # Screen 2: Altitude, Speed, Heading, Sats
            dsp.text("ALT: " + str(round(GPSdata['altitude'], 1)) + "m", 0, 0)
            dsp.text("SPD: " + str(round(GPSdata['knots'], 1)) + " kts", 0, 16)
            dsp.text("HDG: " + str(round(GPSdata['heading'], 1)), 0, 32)
            dsp.text("SAT: " + str(GPSdata['sats']), 0, 48)
...
try:
    while True:
        ...
        if GPSdata['fix'] == False:
            ...
            print("GPS Report:")
            print("Date:", GPSdata['date'], "Time:", GPSdata['time'])
            print("Lat:", GPSdata['latDD'], "Lon:", GPSdata['lonDD'])
            print("Alt:", GPSdata['altitude'], "Knots:", GPSdata['knots'])
            print("Heading:", GPSdata['heading'], "Sats:", GPSdata['sats'])
            print()
